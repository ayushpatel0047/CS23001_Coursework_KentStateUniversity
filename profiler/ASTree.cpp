...
AST::AST(const AST& actual) {
    ...
    text = actual.text;
    for (AST* c : actual.child) {
        child.push_back(new AST(*c));  // recursively copy
    }
}
...
void AST::swap(AST& rhs) {
    ...
    child.swap(rhs.child);  // swaps the lists in constant time
}
...
void AST::insertMainHeader(const std::vector<std::string>& profileName,
    ...
    // Insert #include after any initial tokens or whitespace
    AST* includeNode = new AST(token, "#include \"profile.hpp\"\n");
    child.insert(it, includeNode);

    // Insert profile object declarations
    for (size_t i = 0; i < profileName.size(); ++i) {
        std::string decl = "profile " + profileName[i] + "(\"" + fileName[i] + "\");\n";
        AST* declNode = new AST(token, decl);
        child.insert(it, declNode);
    }
}
...
void AST::insertFileHeader(const std::string& profileName) {
    ...
    std::string externDecl = "extern profile " + profileName + ";\n";
    AST* externNode = new AST(token, externDecl);
    child.insert(it, externNode);
}
...
void AST::insertMainReport(const std::vector<std::string>& profileName) {
    ...
    // Find the "main" function
     for (AST* c : child) {
        if (c->tag == "function" && c->getName() == "main") {
            AST* block = c->getChild("block");
            if (!block) return;

            AST* blockContent = block->getChild("block_content");
            if (!blockContent) return;

            // Find the return statement by iterating backwards
            std::list<AST*>::iterator it = blockContent->child.end();
            while (it != blockContent->child.begin()) {
                --it;
                if ((*it)->tag == "return") {
                    // Insert profile report before return
                    for (const auto& pName : profileName) {
                        std::string report = "std::cout << " + pName + " << std::endl;\n";
                        AST* reportNode = new AST(token, report);
                        blockContent->child.insert(it, reportNode);
                    }
                    return;  // done after inserting for all profiles
                }
            }

            // If no return found, append at end
            for (const auto& pName : profileName) {
                std::string report = "std::cout << " + pName + " << std::endl;\n";
                AST* reportNode = new AST(token, report);
                blockContent->child.push_back(reportNode);
            }
            return;
...
void AST::insertFunctionCount(const std::string& profileName) {
    ...
    for (AST* c : child) {
        if (c->tag == "function" || c->tag == "constructor" || c->tag == "destructor") {
            std::string funcName = c->getName();
            AST* block = c->getChild("block");
            if (block) {
                AST* blockContent = block->getChild("block_content");
                if (blockContent) {
                    // Insert the count line as the first statement
                    std::string countStr = profileName + ".count(__LINE__, \"" + funcName + "\");\n";
                    AST* countNode = new AST(token, countStr);
                    blockContent->child.push_front(countNode);
                }
            }
        } else {
            c->insertFunctionCount(profileName);
        }
    }
...
void AST::insertLineCount(const std::string& profileName) {
    ...
    for (auto it = child.begin(); it != child.end(); ++it) {
        ...
        if (c->nodeType == category) {
            if (c->tag == "expr_stmt" || c->tag == "call") {
                // Insert a line count after the statement
                std::string countStr = profileName + ".count(__LINE__);\n";
                AST* countNode = new AST(token, countStr);
                ++it; // move iterator to insert after current
                child.insert(it, countNode);
                --it; // restore iterator position
            } else if (!isStopTag(c->tag)) {
                c->insertLineCount(profileName);
            }
        }
    }
